...
const T = function () {
    ...
    new MutationObserver((t) => {
        for (const o of t) if (o.type === "childList") for (const r of o.addedNodes) r.tagName === "LINK" && r.rel === "modulepreload" && u(r);
...
function A(g) {
    ...
    const h = () => {
        ...
        x = (e) => {
            e.el.querySelector(".fashion-slider-button-next").addEventListener("click", h), e.el.querySelector(".fashion-slider-button-prev").addEventListener("click", p);
        },
        E = (e) => {
            e.el.querySelector(".fashion-slider-button-next").removeEventListener("click", h), e.el.querySelector(".fashion-slider-button-prev").removeEventListener("click", p);
        };
    let y;
    return (
        (r = new F(m, {
            ...
            on: {
                ...
                transitionStart(e) {
                    ...
                    (d.style["background-color"] = k), (b.style.transform = "scale(0.6)"), (i.style.transitionDuration = "1000ms"), (i.style.transform = "scale(1.2)");
                    const L = f.querySelector(".fashion-slider-title-text");
                    (L.style.transition = "1000ms"), (L.style.color = "rgba(255,255,255,0)");
                    const q = (N) => {
                        N.target === i &&
                ...
                transitionEnd(e) {
                    const { slides: c, activeIndex: n, el: s } = e,
                        ...
                        v = e.params.loop;
                    (l.querySelector(".fashion-slider-scale").style.transform = "scale(1)"), (d.style.transitionDuration = "1000ms"), (d.style.transform = "scale(1)");
                    const f = l.querySelector(".fashion-slider-title-text");
                    (f.style.transition = "1000ms"), (f.style.color = "rgba(255,255,255,1)");
                    const b = (i) => {
                        i.target === d && (d.removeEventListener("transitionend", b), (a = !1));
                    };
                    d.addEventListener("transitionend", b),
                        v ||
                            ...
                            n === c.length - 1
                                ? s.querySelector(".fashion-slider-button-next").classList.add("fashion-slider-button-disabled")
                                : s.querySelector(".fashion-slider-button-next").classList.remove("fashion-slider-button-disabled"));
                },
                beforeInit(e) {
                    const { el: c } = e;
                    o(c);
